...
function ChildHandler(proc) {
  ...
  proc.stdout.addListener("data", function (chunk) {
    self.input += chunk;
    
    if (self.input.length > common.MAX_INPUT_SIZE) 
      self.callback({ ok: false
                    , reason: common.ERR_INPUT_TOO_LARGE 
                    });
    
    var match, response;
    
    // Each response is a CRLF-delimited JSON object.

    while (match = self.input.match(common.LINE_REGEX)) {        
      self.input = self.input.substr(match[0].length);

      var responseJSON = match[1];
      
      if (common.debugMode)
        sys.debug("<-- pid " + self.pid + ": " + responseJSON);
      
      try {
        response = JSON.parse(responseJSON);
      } catch (e) {
        self.callback({ ok: false
                      , reason: common.ERR_MALFORMED + ": invalid JSON response"
                      });                    
      }
  ...
  proc.addListener("exit", function (code) {
    if (!this.suicide && !this.available && self.callback) 
      self.callback({ ok: false
