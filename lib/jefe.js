...
function Jefe(options) {
  ...
  this.pool.addListener("spawn", function (childProc) {
    if (common.debugMode) sys.debug("spawned pid " + childProc.pid);
    var childHandler = new ChildHandler(childProc);
    self.childHandlers[childProc.pid] = childHandler;
    childProc.addListener("exit", function () {
      sys.debug("pid " + childProc.pid + " exited");
      delete self.childHandlers[childProc.pid];
    });
...
Jefe.prototype._runRequestOnChild = function (request, childHandler) {
  ...
  // Watch elapsed wall clock time. Kill child if script does not return in time.

  if (script.maxTime > 0) {
    childHandler.killTimer = setTimeout(function () {
      if (typeof childHandler.memWatcher != "undefined") {
        clearInterval(childHandler.memWatcher);
  ...
  // Watch (peak RSS) memory growth of the child.  Kill child if it uses too much memory.
  ...
  if (script.maxMem > 0 || script.maxMemPercent > 0) {
    childHandler.peakMemoryUsage(function (err, initialMem, initialMemPercent) {
      if (err || initialMem == 0 || initialMemPercent == 0) {
        ...
        self._watchMemoryUsage(request, childHandler, initialMem, initialMemPercent);
      }
    });
  }

  // Run the script in the given child process.
  ...
  childHandler.run(script.name, request.sandbox, function (response) {
    ...
    clearTimeout(childHandler.killTimer);
    delete childHandler.killTimer;
    clearInterval(childHandler.memWatcher);
    delete childHandler.memWatcher;
    ...
    // callback expects (error, sandboxIn, sandboxOut) where error and sandboxOut are
    // mutually exclusive.
    ...
    // maybe dispatch more work.

    self._dispatchQueued();
  });
};

Jefe.prototype._watchMemoryUsage = function (request, childHandler, initialMem, initialMemPercent) {
  var 
    self = this,
    checking = false;

  childHandler.memWatcher = setInterval(function () {
    if (checking) return;
    checking = true;
    ...
    childHandler.peakMemoryUsage(function (err, mem, memPercent) {
      if ((script.maxMem        > 0 && mem        - initialMem        > script.maxMem) ||
        ...
        if (typeof childHandler.killTimer != "undefined") {
          clearTimeout(childHandler.killTimer);
          delete childHandler.killTimer;
        }
