...
function Jefe(options) {
  ...
  this.pool = new ChildPool(process.argv[0], [ opts.runner || (__dirname + "/runner.js") ],
                            {minProcs:opts.minProcs, maxProcs:opts.maxProcs});
  this.childHandlers = {};
...
// The dispatcher regularly runs _dispatchQueued to look for not yet dispatched
// work. When a child process finishes, it calls _dispatchQueued directly
// to get a new job more quickly. The dispatcher is used in case a child was
// killed or in case of the first call to run(). 
Jefe.prototype.startDispatcher = function() {
    if (this.dispatcher === null)
        this._dispatchLoop();
}
Jefe.prototype.stopDispatcher = function() {
    if (this.dispatcher !== null) {
        clearTimeout(this.dispatcher);
        this.dispatcher = null;
    }
}
Jefe.prototype._dispatchLoop = function() {
    this._dispatchQueued();
    var self = this;
    this.dispatcher = setTimeout(function() {
        self._dispatchLoop() }, 200);
}
